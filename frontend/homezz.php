<?php
if(!User::isLoggedIn()){
	header("Location: https://imperfectandcompany.com/private/public_html/login");
}



function get_connection(){
  $dsn = "mysql:host=localhost;dbname=igfastdl_2due";
  $user = "igfastdl";
  $passwd = "UR5WnRgRgUcT9mh5";
...
function save_task($type, $task, $id){
  $conn = get_connection();
  if($id){
    $sql = "UPDATE board SET `task`=? WHERE id=?"; // create sql
    $query = $conn->prepare($sql); // prepare
    $query->execute([$task, $id]); // execute
    return $id;
  }else{
    $sql = "INSERT INTO board(`task`,`type`) VALUES (?,?)"; // create sql
    $query = $conn->prepare($sql); // prepare
...
function move_task($id, $position){
  $conn = get_connection();
  $sql = "UPDATE board SET `type`=? WHERE id=?"; // create sql
  $query = $conn->prepare($sql); // prepare
...
function get_tasks($type){
  ...
  try {
    $conn = get_connection();
    $query = $conn->prepare("SELECT * from board WHERE type=? order by id desc");
    $query->execute([$type]);
...
function get_task($id){
  ...
  try {
    $conn = get_connection();
    $query = $conn->prepare("SELECT * from board WHERE id=?");
    $query->execute([$id]);
    $results = $query->fetchAll();
    $results = $results[0];
  }catch (Exception $e){
...
function show_tile($taskObject, $type=""){
  $baseUrl = "./home?shift&id=".$taskObject["id"]."&type=";
...
if(isset($_GET['delete'])){
  ...
  if($id){
    try {
      $conn = get_connection();
      $query = $conn->prepare("DELETE from board WHERE id=?");
      $query->execute([$id]);
      header("Location: ". $_SERVER['PHP_SELF']);
    }catch (Exception $e){
